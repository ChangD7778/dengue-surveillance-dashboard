...
def surveillance_recommendation(risk_df: pd.DataFrame, trend_slope: float, horizon_mode: int, watch_enabled: bool, threshold: float) -> dict:
    ...
    if adjusted and not decision_window.empty:
        ...
        if in_watch_window and not baseline:
            reason = "Pre-season surveillance trigger: medium/high risk + positive trend"
...
# Pipeline
# -----------------------------
st.title("NEA Dengue Surveillance and Early Warning Dashboard")
st.caption("SARIMA + STL dashboard for dengue surveillance and early warning. Data source: Singapore MOH weekly dengue cases.")
...
col2.metric("Recommended posture", rec["action"])
